...
function updateWishlist(itemName, gemCost, quantity) {
    ...
    if (!qty || qty <= 0) {
        ...
        if (existing) {
            ...
            existing.textContent = `${itemName} x${qty} - ${(gemCost * qty).toLocaleString('en-US')} Gems`; // Updated to 'en-US'
            total += gemCost * qty;
        } else {
            ...
            li.textContent = `${itemName} x${qty} - ${(gemCost * qty).toLocaleString('en-US')} Gems`; // Updated to 'en-US'
